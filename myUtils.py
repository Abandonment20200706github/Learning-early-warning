...
def f1_knowledgePoint():
    ...
    df_knowledgePoint = pd.read_sql(sql, Constants.conn)
    ...
    df_knowledgePoint['front_subchapter'] = df_knowledgePoint['front_subchapter'].apply(f)
...
df_sectionAndKnowledge = pd.read_sql(sql,Constants.conn)
def f(lessonid):
...
def f2_knowledgePoint2sections():
    ...
    df_nowledgePoint['front_subchapter'] = df_nowledgePoint['front_subchapter'].apply(
        f1_knowledgePoint2SectionSolo)
